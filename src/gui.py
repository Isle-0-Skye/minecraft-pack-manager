...
try:
    ...
    print_import_error(
        import_tree="[gui]<-[PyQt6.QtWidgets]",
        error=error,
        link="https://pypi.org/project/PyQt6",
        text="PyQt6",
    )
...
try:
    ...
    print_import_error(
        import_tree="[gui]<-[..modules.common]",
        error=error,
        import_type="photon",
        hypertext=False,
    )
...
try:
    from .logic import (
        ...
        get_QPixmap,
        open_launcher,
        refresh_transfer_options,
        reload_player_labels,
        start_transfer,
        user_list,
        validate_settings,
        write_settings_to_gui,
    )
except ImportError as error:
    print_import_error(
        import_tree="[gui]<-[.logic]",
        error=error,
        import_type="Project",
        hypertext=False,
    )
...
class workerThread(QThread):
    ...
    def __init__(self, run_function, args=None, parent=None) -> None:
    ...
    def run(self) -> None:
        pLOG.debug(f"worker {self} started")
        if self.args is None:
            self.run_function()
        else:
            self.run_function(self.args)
        pLOG.debug(f"worker {self} finished")
        self.finished.emit()
...
class notifyDialog(QDialog):
    def __init__(
        ...
        self.close_button.clicked.connect(lambda: [self.close(), pLOG.debug(f"close {self}")])
...
class MPM(QApplication):
    def __init__(self, argv: list[str]) -> None:
        ...
        # < create window >#
        pLOG.debug("window_setup")
        window = self.window_setup()

        # < add any fonts provided by mpm or photon >#
        pLOG.debug("fonts")
        local_font_directory = os.path.join(
            UNIVERSALS.package_root(PROJECT_MANIFEST.name()), "resources", "fonts"
        )
        self.add_font(local_font_directory)

        photon_font_directory = os.path.join(UNIVERSALS.root(), "resources", "fonts")
        self.add_font(photon_font_directory)

        # < apply sheet >#
        pLOG.debug("styling")
        if os.path.exists(LOCAL_SETTINGS.style_sheet()):
            self.setStyleSheet(LOCAL_SETTINGS.style_sheet_data())
        else:
            pLOG.warning("no style sheet found")
    ...
    def window_setup(self):
        ...
        # < window icon >#
        pLOG.debug("window_setup: icons")
        self.setWindowIcon(get_QIcon(IMAGES_VAULT.images_dict()["general"]["icon"]))

        # < window title >#
        pLOG.debug("window_setup: title")
        display_name = PROJECT_MANIFEST.display_name()
        version = PROJECT_MANIFEST.version()
        pLOG.debug(f"display_name: {display_name}, version: {version}")
        ...
        pLOG.debug("window_setup: geometry")
    ...
    def add_font(self, directory_path):
        ...
        pLOG.debug(f"loading fonts from {directory_path}")
...
class Container(QWidget):
    def __init__(
        ...
        self.background_label.setPixmap(
            get_QPixmap(IMAGES_VAULT.images_dict()["general"]["background"], size=(width, height))
        )
        self.background_label.setMinimumSize(1, 1)
        layout.addWidget(self.background_label, 0, 0, Qt.AlignmentFlag.AlignCenter)

        # < pages >#
        self.pages.setContentsMargins(0, 0, 0, 0)
        self.pages.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.pages.setFixedSize(width, height)

        for page in [HomePage, UploadPage, DownloadPage, SettingsPage, HelpPage, PlayPage]:
...
class HomePage(QWidget):
    def __init__(self, parent):
        ...
        layout.addWidget(
            self.title_label, 0, 0, 1, 4, Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignTop
        )
        ...
        row = 1
        column = 0
        for button in [" Upload", " Download", " Settings", " Help", " Play", " Quit"]:
            self.button = QPushButton(button)
            self.button.setFixedSize(256, 72)
            self.button.setObjectName("home_buttons")

            icon_name = f"{button.strip().lower()}_button"
            self.button.setIcon(get_QIcon(IMAGES_VAULT.images_dict()["home_page"][icon_name]))
            self.button.setIconSize(QSize(56, 56))

            buttons[button.strip().lower()] = self.button
            layout.addWidget(self.button, row, column, 1, 2, Qt.AlignmentFlag.AlignCenter)
...
class UploadPage(QWidget):
    def __init__(self, parent):
        ...
        self.source_label.setFixedHeight(40)
        self.source_label.setMinimumWidth(160)
        self.source_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        layout.addWidget(
            self.source_label, 1, 0, 1, 1, Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignLeft
        )

        # < src >#
        self.source_combobox = QComboBox()
        self.source_combobox.setFixedHeight(40)
        self.source_combobox.setMinimumWidth(384)
        layout.addWidget(
        ...
        self.destination_label.setMinimumWidth(160)
        ...
        self.destination_combobox.setFixedHeight(40)
        self.destination_combobox.setMinimumWidth(384)
        layout.addWidget(
        ...
        self.refresh_button.clicked.connect(lambda: [self.refresh_options()])
        layout.addWidget(self.refresh_button, 4, 0, 1, 1)

        # < continue >#
        self.continue_button = QPushButton("Continue")
        self.continue_button.setFixedSize(160, 40)
        self.continue_button.clicked.connect(lambda: [self.transfer()])
        layout.addWidget(self.continue_button, 4, 1, 1, 2, Qt.AlignmentFlag.AlignCenter)
    ...
    def refresh_options(self):
        self.worker1 = workerThread(refresh_transfer_options, self)

        popup = notifyDialog("Updating Transfers List", "Updating...", parent=self, modal=True)
        popup.close_button.hide()

        self.worker1.finished.connect(self.worker1.quit)
        self.worker1.finished.connect(popup.close)

        popup.show()
        self.worker1.start()

    def transfer(self):
        self.worker1 = workerThread(start_transfer, self)

        popup = notifyDialog(
            "Uploading files...", "Transfering...\nPlease wait", parent=self, modal=True
        )
        popup.close_button.hide()
...
class SettingsPage(QWidget):
    def __init__(self, parent):
        ...
        self.launcher_path_validate.setIcon(
        ...
        self.launcher_path_validate.clicked.connect(
            lambda: [self.settings_popup("launcher_path is valid")]
        )
        layout.addWidget(self.launcher_path_validate, 1, 3, 1, 1, Qt.AlignmentFlag.AlignRight)
        ...
        self.instances_path_validate.setIcon(
            get_QIcon(IMAGES_VAULT.images_dict()["settings_page"]["valid"])
        )
        self.instances_path_validate.setIconSize(QSize(24, 24))

        self.instances_path_validate.clicked.connect(
            lambda: [self.settings_popup("instances_path is valid")]
        )
        layout.addWidget(self.instances_path_validate, 2, 3, 1, 1, Qt.AlignmentFlag.AlignRight)
        ...
        self.verify_settings_button.clicked.connect(
            lambda: [validate_settings(self, self.setting_widgets, self.icons, write=True)]
        )
        ...
        self.setLayout(layout)
        write_settings_to_gui(self.setting_widgets)
        validate_settings(self, self.setting_widgets, self.icons)
    ...
    def settings_popup(
        ...
        popup = settingsPopup(self, label_text, second_button, second_button_text)
        if popup.exec() and second_button:
            ...
            if len(split_path) < 3:
                pLOG.warning("Failed to make file path: too short")
...
class PlayPage(QWidget):
    def __init__(self, parent):
        ...
        self.open_launcher_button.clicked.connect(lambda: [open_launcher()])
        layout.addWidget(self.open_launcher_button, 2, 0, Qt.AlignmentFlag.AlignCenter)

        # < user refresh button >#
        self.refresh_user_list_button = QPushButton("Update List")
        self.refresh_user_list_button.setFixedSize(200, 40)
        self.refresh_user_list_button.clicked.connect(lambda: [self.update_list()])
        layout.addWidget(self.refresh_user_list_button, 2, 1, Qt.AlignmentFlag.AlignCenter)

        # < user add button >#
        self.add_user_button = QPushButton()
        self.add_user_button.setFixedSize(80, 40)
        self.add_user_button.setIcon(
            get_QIcon(IMAGES_VAULT.images_dict()["play_page"]["plus_slash_minus"], size=(60, 60))
        )
        self.add_user_button.setIconSize(QSize(60, 60))
        self.add_user_button.clicked.connect(lambda: [self.change_users()])
        layout.addWidget(
            self.add_user_button, 2, 2, Qt.AlignmentFlag.AlignCenter | Qt.AlignmentFlag.AlignLeft
        )
        ...
        self.home_button.clicked.connect(lambda: [parent.change_page(0)])
        layout.addWidget(self.home_button, 2, 3, 1, 1, Qt.AlignmentFlag.AlignCenter)

        self.setLayout(layout)

    def update_list(self):
        self.worker1 = workerThread(get_player_skin)
        self.worker2 = workerThread(reload_player_labels, self.players_tab)

        popup = notifyDialog("Updating User List", "Updating...", parent=self, modal=True)
        popup.close_button.hide()
    ...
    def change_users(self):
        ...
        if popup.exec():
            edit_users(popup)
            reload_player_labels(self.players_tab)
...
class removeUserTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.username_input.setFixedHeight(40)
        self.username_input.addItems(user_list())
        layout.addWidget(self.username_input, 0, 1, 1, 2)
